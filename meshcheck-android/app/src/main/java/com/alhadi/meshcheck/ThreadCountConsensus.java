...
/** Robust fusion of multiple scan lines across a calibrated physical window. */
public final class ThreadCountConsensus {
    ...
    public static FrameResult fuse(ThreadProfileCounter.Result[] results) {
        return fuse(results, 1.0f);
    }

    /** Fuse scan lines measured over a physical window length in centimeters. */
    public static FrameResult fuse(ThreadProfileCounter.Result[] results, float physicalLengthCm) {
        if (results == null || results.length == 0 || !(physicalLengthCm > 0f)) {
            return FrameResult.fail("لا توجد خطوط مسح.", results == null ? 0 : results.length);
        }

        List<ThreadProfileCounter.Result> valid = new ArrayList<>();
        for (ThreadProfileCounter.Result r : results) {
            if (r != null && r.ok && r.fullLineCount >= 3 && r.spacingThreadsPerCm > 0f && r.confidence >= 0.15f) valid.add(r);
        }
        int minimum = Math.max(3, (results.length + 1) / 2);
        if (valid.size() < minimum) {
            return FrameResult.fail("لم تتفق خطوط المسح — حسّن التركيز والإضاءة.", results.length);
        }
        ...
        if (countInliers.size() < minimum) {
            return FrameResult.fail("العد مختلف بين أجزاء منطقة القياس — اجعل القماش مسطحًا وثابتًا.", results.length);
        }
        ...
        float weightedDensity = 0f, densityWeight = 0f, weightedFullCount = 0f, fullCountWeight = 0f;
        float confidenceSum = 0f;
        int densityInliers = 0;
        float minDensity = Float.MAX_VALUE, maxDensity = -Float.MAX_VALUE;
        ...
        for (ThreadProfileCounter.Result r : countInliers) {
            ...
            float expectedFullLines = r.spacingThreadsPerCm * physicalLengthCm;
            float edgeDifference = Math.abs(expectedFullLines - r.fullLineCount);
    ...
    public static final class Stabilizer {
        ...
        public synchronized Snapshot push(FrameResult frame) {
            ...
            if (previousCenters.length >= 3 && frame.centersNormalized.length >= 3) {
                ...
                if (motion > 0.018f) return snapshot(false, "HOLD — vibration detected", frame.currentFullLineCount);
            } else previousCenters = frame.centersNormalized.clone();
        ...
        private Snapshot snapshot(boolean accepted, String reason, int currentFullLineCount) {
            ...
            int from = trim, to = values.length - trim;
            float sum = 0f;
            for (int i = from; i < to; i++) sum += values[i];
            float mean = sum / Math.max(1, to - from);
            float low = values[from], high = values[to - 1], spread = high - low;
            ...
            boolean stable = densityHistory.size() >= minimumStableSamples
                    && spread <= tolerance && confidence >= 0.42f;
